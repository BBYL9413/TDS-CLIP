...
def conv_3xnxn_std(inp, oup, kernel_size=3, stride=3, groups=1):
    return nn.Conv3d(inp, oup, (3, kernel_size, kernel_size), (1, stride, stride), (1, 1, 1), groups=groups)
...
class SAModule(nn.Module):
    ...
    def __init__(self, channel, reduction=16, n_segment=1):
        ...
        self.conv2 = nn.Conv2d(
            ...
            padding=0,
            bias=False)
        self.bn2 = nn.BatchNorm2d(num_features=self.channel)
        
        self.sigmoid_forward = nn.Sigmoid()

    def forward(self, x):
       
        x = rearrange(x, 'b c t h w -> (b t) c h w', t= self.n_segment)
        bottleneck = self.conv1(x)  # nt, c//r, h, w
        bottleneck = self.bn1(bottleneck)  # nt, c//r, h, w
       
        bottleneck = rearrange(bottleneck, '(b t) c h w -> b c t h w', t= self.n_segment)
        t_tea = self.pool(bottleneck)  # nt, c//r, h, w
      
        diff_fea = bottleneck - t_tea  # nt, c//r, h, w

        mutiview_y1 = diff_fea
        mutiview_y2 = torch.mean(diff_fea, dim=1, keepdim=True) # nt, 1, h, w
    
        y = 0.5*mutiview_y1 + 0.5*mutiview_y2
        ...
        output = x + x * y
        output = rearrange(output, '(b t) c h w -> b c t h w', t= self.n_segment)
        return output
...
class AttnCBlock(nn.Module):
    def __init__(self, dim, side_dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.T = T
        self.mse = SAModule(side_dim, n_segment=self.T)
    ...
    def forward(self, x, x_token=None, side_position_embeddings=None, layer_id=None, use_ckpt=False):
        ...
        if use_ckpt:
            ...
            x = x + self.drop_path(self.mse(conv_out))
        else:
            x = x + self.drop_path(self.mse(self.conv(self.bn_1(x))))
...
class Transformer2(nn.Module):
    ...
    def forward(self, x: torch.Tensor, side_spatial_position_embeddings: torch.Tensor=None):
        k = 0
        h = int(x.shape[0] ** 0.5)
        for i in range(len(self.resblocks)):
            x = self.resblocks[i](x)
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,dropout = None,joint=False, emb_dropout = 0., T=8, side_dim=384, num_diff=1):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.side_conv1_c5 = conv_3xnxn_std(12, self.side_dim, kernel_size=patch_size, stride=patch_size)
        self.side_pre_bn3d_c5 = nn.BatchNorm3d(self.side_dim)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_c5 = torch.cat([x2-x1,x3-x2,x4-x3,x5-x4],1).view(-1,12,x2.size()[2],x2.size()[3])
        
        x_c5 = rearrange(x_c5, '(b t) c h w -> b c t h w', t=self.T)
        x_c5_side = self.side_pre_bn3d_c5(self.side_conv1_c5(x_c5))
       
        x_side = rearrange(x3, '(b t) c h w -> b c t h w', t=self.T)

        x = self.conv1(x3)  # shape = [*, width, grid, grid]
        ...
        #x_c5_side = x_c5_side.permute(1, 0, 2)
...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 joint=False,
                 tm=None, T=8,dropout = 0., emb_dropout = 0.,side_dim = 384, num_diff=1
                 ):
        super().__init__()
...
def build_model(state_dict: dict, tm=None,T=8,dropout=0., joint=False,emb_dropout=0.,pretrain=True, side_dim=384, num_diff=1):
    ...
    model = CLIP(
        ...
        dropout=dropout, emb_dropout=emb_dropout,side_dim=side_dim,num_diff=num_diff
    )

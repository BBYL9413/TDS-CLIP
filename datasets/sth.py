...
class Video_dataset(data.Dataset):
    ...
    def _parse_list(self):
        ...
        #     self.video_list = self.video_list[:int(len(self.video_list) * data_percent)]
    ...
    def _sample_indices(self, video_list):
        if self.dense_sample:
            ...
            ######### slowfast ###########
            # seg_size = len(video_list) // self.num_segments
            # offsets = []
            # if seg_size > 0:
            #     offsets += list(np.multiply(list(range(self.num_segments)), seg_size) + np.random.randint(seg_size, size=self.num_segments))
            # elif seg_size > self.num_segments:
            #     offsets += list(np.sort(np.random.randint(len(video_list), size=self.num_segments)))
            # else:
    ...
    def _get_test_indices(self, video_list):
        if self.dense_sample:
            ...
            # all_index = []
            # tick = len(video_list) / float(self.num_segments)
            # all_index = list(np.array([int(tick / 2.0 + tick * x) for x in range(self.num_segments)] + [int(tick * x) for x in range(self.num_segments)]))
            # while len(all_index) < (self.num_segments * num_clips):
            #     all_index.append(all_index[-1])
            # return np.array(all_index) + self.index_bias
    ...
    def get(self, record, video_list, indices):
        ...
        half = self.num_diff // 2
        indices = indices - half
        for seg_ind in indices:
            p = int(seg_ind)
            for i in range(0, self.num_diff, 1):     
                if self.modality == 'video':

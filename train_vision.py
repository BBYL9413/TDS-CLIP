...
class VideoCLIP(nn.Module):
    def __init__(self, clip_model, fusion_model, config) :
        ...
        self.num_diff = config.data.num_diff
    def forward(self, image):
        image = rearrange(image, '(b t m) c h w -> (b t) (m c) h w',t=self.n_seg, m=self.num_diff)
...
def main(args):
    ...
    if model_name in ["EVA02-CLIP-L-14", "EVA02-CLIP-L-14-336", "EVA02-CLIP-bigE-14", "EVA02-CLIP-bigE-14-plus"]:
        ...
        model, _, preprocess = create_model_and_transforms(model_name, pretrained=weight_path[model_name], force_custom_clip=True, T=config.data.num_segments, side_dim=config.network.side_dim, num_diff=config.data.num_diff)
    ...
    if config.data.num_diff % 2 == 0:
        raise ValueError("Odd numbers only.")
    ...
    train_data = Video_dataset(
        ...
        transform=transform_train, dense_sample=config.data.dense, num_sample=config.data.num_sample, num_diff=config.data.num_diff)
    ...
    val_data = Video_dataset(
        ...
        transform=transform_val, dense_sample=config.data.dense,num_diff=config.data.num_diff)
    ...
    if args.distributed:
        model_onehot = DistributedDataParallel(model_onehot.cuda(), device_ids=[args.gpu], find_unused_parameters=True)
